...
public class GithubRepositoryControllerTest {
    ...
    public void shouldReturnSuccessRepositoriesByUsername() throws Exception {
        ...
        mockMvc.perform(
                get("/api/github/users/{username}/repositories", username).contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk()).andExpect(jsonPath("$").isArray()).andExpect(jsonPath("$", hasSize(1)))
            .andExpect(jsonPath("$[0].name").value("akka")).andExpect(jsonPath("$[0].login").value("David"))
    ...
    public void shouldReturnNotFoundByUsername() throws Exception {
        ...
        mockMvc.perform(
                get("/api/github/users/{username}/repositories", notFoundUserName).contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    public void shouldReturnErrorWhenHeaderAcceptNotApplicationJSON() throws Exception {
        String username = "David";

        mockMvc.perform(
                get("/api/github/users/{username}/repositories", username).contentType(MediaType.APPLICATION_JSON)
                    .accept(MediaType.APPLICATION_XML_VALUE)).andExpect(status().isNotAcceptable())
            .andExpect(jsonPath("$.status").value("406")).andExpect(jsonPath("$.message").value(
    ...
    private void initMockServerNegativeResponse(String notFoundUserName, String notFoundUserMsg) {
        mockRestServiceServer.expect(requestTo(properties.getFullReposUrl(notFoundUserName)))
            .andRespond(withStatus(HttpStatus.NOT_FOUND).body(notFoundUserMsg).contentType(MediaType.APPLICATION_JSON));
    }

    @SneakyThrows
    private void initMockServerResponse(String username, String branchName) {
    ...
    private List<RepositoryDto> mockReposDataFromGithub(String username) {
        return Arrays.asList(new RepositoryDto("akka", new OwnerDto(username), false),
                             new RepositoryDto("akkaCustom", new OwnerDto(username), true));
    }

    private List<BranchDto> mockBranchesDataFromGithub() {
        return Arrays.asList(new BranchDto("Master", new CommitDto("ascklefgj3409ru89dasuc98sdoivjowi")),
